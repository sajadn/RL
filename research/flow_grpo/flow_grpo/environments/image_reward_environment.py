        ...
        for meta in metadata:
            if "ground_truth" not in meta:
                raise KeyError(
                    ...
                    "(produced by research/flow_grpo/flow_grpo/data/export_ocr_prompts.py)"
                )
        arr: Any = (
        ...
        for meta in metadata:
            if "ground_truth" not in meta:
                raise KeyError(
                    ...
                    "metadata (produced by research/flow_grpo/flow_grpo/data/export_ocr_prompts.py)"
                )
        arr: Any = (
            (images * 255).round().clamp(0, 255).to(torch.uint8).cpu().numpy()
        ).transpose(0, 2, 3, 1)
...
class ImageRewardEnvironment:
    ...
    def __init__(
        self,
        config: ImageRewardEnvConfig | dict[str, Any],
        *,
