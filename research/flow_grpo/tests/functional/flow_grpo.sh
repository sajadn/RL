...
#
# Discovered by the shared research functional-test runner.
# Usage from the repository root:
#   bash research/flow_grpo/tests/functional/flow_grpo.sh
set -euo pipefail
PROJECT_ROOT=$(cd "$(dirname "$0")/../.." && pwd)
NEMO_RL_ROOT=$(realpath "$PROJECT_ROOT/../..")
cd "$PROJECT_ROOT"
uv sync --locked --inexact --group test
PY=${NRL_PYTHON:-uv run --locked --no-sync python}
...
uv run "$NEMO_RL_ROOT"/tests/json_dump_tb_logs.py "$LOG_DIR" --output_path "$JSON_METRICS"
