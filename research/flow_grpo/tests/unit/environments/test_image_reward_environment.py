...
import ray
import torch
from flow_grpo.environments.image_reward_environment import (
    _PLUGIN_REGISTRY,
    DummyImageReward,
...
def test_reward_workers_receive_runtime_environment(monkeypatch):
    import flow_grpo.environments.image_reward_environment as module
    ...
    class FakeWorker:
        ...
        def remote(self, factory):
            return object()

    monkeypatch.setattr(module, "_RewardWorker", FakeWorker)
    runtime_env = {"py_executable": "/research-venv/bin/python"}
    ImageRewardEnvironment(
        {"plugins": [{"name": "dummy"}], "num_workers_per_plugin": 2},
        runtime_env=runtime_env,
    )
    assert len(calls) == 2
    assert all(call["runtime_env"] == runtime_env for call in calls)
...
def test_image_reward_environment_two_plugins_sum(ray_init_and_shutdown):  # noqa: F811
    ...
    try:
        ...
        from flow_grpo.environments.image_reward_environment import _PLUGIN_REGISTRY
...
def test_ocr_edit_distance_score_semantics():
    from flow_grpo.environments.image_reward_environment import ocr_edit_distance_score
...
def test_ocr_reward_plugin_with_injected_engine():
    import torch
    from flow_grpo.environments.image_reward_environment import OcrEditDistanceReward
...
def test_genrm_ocr_prompt_matches_verl_omni_verbatim():
    from flow_grpo.environments.image_reward_environment import GENRM_OCR_PROMPT
...
def test_levenshtein_matches_reference_implementation():
    from flow_grpo.environments.image_reward_environment import _levenshtein
...
def test_genrm_ocr_score_semantics():
    from flow_grpo.environments.image_reward_environment import genrm_ocr_score
...
def test_genrm_ocr_sampling_params_configurable(monkeypatch):
    from flow_grpo.environments.image_reward_environment import GenRmOcrReward
