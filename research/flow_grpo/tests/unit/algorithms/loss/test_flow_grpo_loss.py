...
import pytest
import torch
from flow_grpo.algorithms.loss.flow_grpo import FlowGRPOLossConfig, FlowGRPOLossFn


def _cfg(beta: float = 0.0, adv_clip_max: float | None = 5.0) -> FlowGRPOLossConfig:
    return FlowGRPOLossConfig(
        ratio_clip_min=0.2,
        ratio_clip_max=0.2,
...
def test_zero_advantage_yields_zero_policy_loss():
    ...
    loss, metrics = fn(curr, gen, adv, timestep_mask)
...
def test_kl_term_is_quadratic_in_mean_offset():
    fn = FlowGRPOLossFn(_cfg(beta=1.0))
    B, T, C, H, W = 1, 1, 1, 2, 2
    curr = torch.zeros(B, T)
    gen = torch.zeros(B, T)
    adv = torch.zeros(B, T)
...
def test_kl_term_disabled_when_beta_zero_even_with_means():
    fn = FlowGRPOLossFn(_cfg(beta=0.0))
    B, T, C, H, W = 1, 1, 1, 2, 2
    loss, metrics = fn(
        torch.zeros(B, T),
        torch.zeros(B, T),
...
def test_advantage_clip_max_applied_before_ratio():
    fn = FlowGRPOLossFn(_cfg(adv_clip_max=2.0))
    B, T = 1, 1
    curr = torch.zeros(B, T)
    gen = torch.zeros(B, T)
    ...
    loss, _ = fn(curr, gen, adv, timestep_mask)
...
def test_advantage_clip_disabled_when_none():
    fn = FlowGRPOLossFn(_cfg(adv_clip_max=None))
    B, T = 1, 1
    curr = torch.zeros(B, T)
    gen = torch.zeros(B, T)
...
def test_loss_is_differentiable_in_curr_logprob(seed: int):
    ...
    fn = FlowGRPOLossFn(_cfg(beta=0.1))
    B, T, C, H, W = 2, 3, 1, 2, 2
