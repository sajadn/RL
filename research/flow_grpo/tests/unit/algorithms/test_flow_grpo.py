...
@pytest.mark.parametrize(
    "config_path",
    sorted((Path(__file__).resolve().parents[3] / "configs").rglob("*.yaml")),
    ids=lambda path: path.stem,
)
def test_project_configs_validate(config_path):
    from flow_grpo.algorithms.flow_grpo import MasterConfig
    from omegaconf import OmegaConf

    from nemo_rl.utils.config import load_config

    config = OmegaConf.to_container(load_config(str(config_path)), resolve=True)
    MasterConfig.model_validate(config)
...
def test_master_config_rejects_kl_with_full_param():
    ...
    cfg = OmegaConf.to_container(
        load_config("configs/recipes/flow_grpo_qwen_image_ocr.yaml"),
        resolve=True,
    )
...
def test_run_validation_passes_generation_overrides():
    import torch
    from flow_grpo.algorithms.flow_grpo import _run_validation
...
def test_build_train_data_slices_to_window_columns():
    import torch
    from flow_grpo.algorithms.flow_grpo import _build_train_data
...
def _mini_batch_algo_cfg(**overrides):
    from flow_grpo.algorithms.flow_grpo import FlowGRPOAlgoConfig
...
def _run_one_train_step(algo_cfg, checkpoint_dir=None, policy=None):
    import torch
    from flow_grpo.algorithms.flow_grpo import flow_grpo_train
...
def test_global_std_tames_constant_group_amplification():
    import torch
    from flow_grpo.algorithms.flow_grpo import _compute_advantages
