...
2. **Reward**: score decoded images with a pluggable reward environment (`research/flow_grpo/flow_grpo/environments/image_reward_environment.py`).
...
4. **Train**: recompute transition log-probs with grad and apply the clipped policy-gradient loss (`research/flow_grpo/flow_grpo/algorithms/loss/flow_grpo.py`), with optional Gaussian KL against the reference policy (`loss_fn.beta > 0`; the reference is the base model with the LoRA adapter disabled).

## Components

| Component | Path |
|---|---|
| SDE step / log-prob kernel | `research/flow_grpo/flow_grpo/algorithms/sde.py` |
| Config schemas and protocols | `research/flow_grpo/flow_grpo/models/interfaces.py` |
| Clipped policy-gradient loss | `research/flow_grpo/flow_grpo/algorithms/loss/flow_grpo.py` |
| Qwen-Image pipeline adapter | `research/flow_grpo/flow_grpo/models/pipeline.py` |
| Ray worker (Automodel loading, LoRA, DP all-reduce) | `research/flow_grpo/flow_grpo/models/policy/workers/flow_grpo_worker.py` |
| Controller-side policy facade | `research/flow_grpo/flow_grpo/models/policy/flow_grpo_policy.py` |
| Training loop | `research/flow_grpo/flow_grpo/algorithms/flow_grpo.py` |
| Prompt dataset | `research/flow_grpo/flow_grpo/data/text_to_image_prompt.py` |
| Reward environment | `research/flow_grpo/flow_grpo/environments/image_reward_environment.py` |
| Entry point | `research/flow_grpo/run_flow_grpo.py` |
...
Run from this research project. Its dependencies include NeMo RL with Automodel, Diffusers, and the OCR reward packages:

```bash
cd research/flow_grpo
...
```bash
uv run --locked --no-sync python -m flow_grpo.data.export_ocr_prompts --out-dir data/ocr
```
...
```bash
uv run --locked --directory ../.. --extra vllm vllm serve Qwen/Qwen3-VL-8B-Instruct \
...
```bash
uv run --locked --no-sync python run_flow_grpo.py \
    --config configs/recipes/flow_grpo_qwen_image_ocr.yaml
```
...
|---|---|---|---|
| Exemplar | `genrm_ocr` | 300 | `research/flow_grpo/configs/recipes/flow_grpo_qwen_image_ocr.yaml` |
| Nightly (CI convergence gate) | `ocr` (PaddleOCR, CPU) | 60 | `research/flow_grpo/configs/recipes/flow_grpo-qwen-image-ocr-1n8g-dp8-lora.yaml` |
...
All defaults live on the pydantic `BaseModel` schemas (`research/flow_grpo/flow_grpo/models/interfaces.py`, `research/flow_grpo/flow_grpo/algorithms/flow_grpo.py`, `research/flow_grpo/flow_grpo/algorithms/loss/flow_grpo.py`) and are documented in the exemplar YAML `research/flow_grpo/configs/recipes/flow_grpo_qwen_image_ocr.yaml`. Key blocks:
...
Reward workers are Ray actors (`num_workers_per_plugin` replicas, CPU by default); custom rewards register via `flow_grpo.environments.image_reward_environment.register_image_reward`.
...
- **Model support**: Qwen-Image, loaded through the NeMo Automodel stack. Other flow-matching pipelines can implement the `DiffusionPipelineAdapter` protocol (`research/flow_grpo/flow_grpo/models/interfaces.py`).
...
This experimental project keeps generation and policy training in the same worker.
It lives in `research/` while customer feedback and integration requirements are
being established. The Python package is `flow_grpo`; core NeMo RL remains a
workspace dependency. The SDE utilities live under `flow_grpo/algorithms/` and the
model adapter under `flow_grpo/models/`. The policy facade and its workers
follow core's layout under `flow_grpo/models/policy/` and
`flow_grpo/models/policy/workers/`, respectively.

The root `uv.lock` is shared by the workspace. After changing this project's
`pyproject.toml`, run `uv lock` from the repository root and review the lockfile
changes. No root `diffusion` extra is needed.

The driver registers both policy and reward actor environments with
`uv run --locked --directory <research-project>`. The existing worker environment
builder first installs root build dependencies, then installs this project's
dependencies into that worker environment. Reward actors receive the resolved
runtime environment explicitly. `NEMO_RL_PY_EXECUTABLES_SYSTEM=1` instead uses the
driver interpreter, which must already have these dependencies installed.

From this directory, run unit tests with:
...
Unit and functional tests are discovered by the shared research CI runners.
The OCR convergence script is listed in the root `tests/test_suites/nightly.txt`;
its matching recipe remains in this project's `configs/recipes/`. Run suite
scripts from the repository root, or use `tools/launch` with their
repository-relative path.

Both YAML configurations live in `configs/recipes/`. The full GenRM recipe has
a matching suite script in `disabled.txt` because it requires an external judge;
the shorter PaddleOCR recipe remains in `nightly.txt`.
