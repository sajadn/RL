...
class DiffuGRPOLossPostProcessor(LossPostProcessor):
    ...
    def __call__(
        ...
        def loss_fn_inner(
            ...
            # Block-aware ESPO: reduce the per-token [N, noisy_len] logprobs to a
            # per-sequence block-aware ELBO scalar (each block reweighted by its
            # realized masking ratio, then divided by the total response length L)
            # and feed the [N_seq] scalars to the loss (Route B): the GSPO
            # sequence-ratio path gives the ESPO ratio and the k2 KL a genuine
            # 0.5*(Lnorm_theta - Lnorm_ref)^2 per sequence. The mask(s) (shared
            # seed) are identical for curr/prev/ref, so the ratio is valid. (k2 is
            # on the NORMALIZED ELBO Lnorm; the paper uses the raw L_hat -- differs
            # by a 1/L^2 constant, absorbed into beta.)
            # ESPO is the antithetic coupled pair (num_mc_samples == 2). Training
            # is SAMPLE-MAJOR: this microbatch holds K whole sequences with their
            # two level rows grouped (level 0 = mask M, level 1 = complement Mbar).
            # We reshape to [K, M], reduce each level with its own harvest mask, and
            # average the two ELBOs (scheme (b)) BEFORE forming the per-sequence
            # ratio. The objective is separable across sequences, so the gradient
            # accumulates over the remaining microbatches (standard NeMo-RL path).
            
            if logprob_estimation_cfg.get("type") == "espo_block_aware":
                response_lengths = data_dict["diffu_grpo_response_lengths"]
